...
jQuery.fn.slugable = function(options) {
    var settings = {
        slugfield: '',
        type: 'underscore'
    };
    ...
    makeSlug = function() {
        if (settings.slugfield != 'underscore') {
            var slugcontent = jQuery.trim(removeDiacritics($this.val()));
            var slugcontent_hyphens = slugcontent.replace(/\s/g, '_');
            var finishedslug = slugcontent_hyphens.replace(/[^a-zA-Z0-9\_]/g, '_');
            finishedslug = finishedslug.replace(/[_]{2,}/g, '_');
            finishedslug = finishedslug.replace(/(^[_])|([_]$)/g, '');
        } else {
            var slugcontent = jQuery.trim(removeDiacritics($this.val()));
            var slugcontent_hyphens = slugcontent.replace(/\s/g, '-');
            var finishedslug = slugcontent_hyphens.replace(/[^a-zA-Z0-9\-]/g, '-');
        ...
        if (finishedslug != '') {
            if (settings.slugfield != '') {
                jQuery(settings.slugfield).val(finishedslug.toLowerCase());
                jQuery(settings.slugfield).trigger('input');
                jQuery(settings.slugfield).trigger('keyup');
            } else {
                jQuery($this).val(finishedslug.toLowerCase());
...
window.onload = function() {
    ...
    $('.tree').each(function() {
        ...
        $(this).select2({
            ...
            placeholder: 'Select',
            closeOnSelect: false,
            formatResult: format_tree,
            formatSelection: format_tree,
            dropdownCss: dropdownCss,
            containerCss: containerCss
        });
    ...
    tinymce.init({
        ...
        selector: "textarea.richtext", theme: "modern", width: 680, height: 300,
        plugins: [
        ...
        external_plugins: {"filemanager": external_filemanager_path+"plugin.min.js"}
    });
}
